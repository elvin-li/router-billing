...
// TestPayQRReadsPayloadFromOrderRow pins the v0.103 security fix: the QR
// payload comes from orders.qr_payload, NOT from a URL parameter.
//
// Pre-v0.103 the handler did:
//
...
func TestPayQRReadsPayloadFromOrderRow(t *testing.T) {
	...
	// Case 2 — attacker passes ?payload=phishing. Pre-v0.103 this would
	...
	if body != body2 {
		t.Errorf("URL-supplied ?payload=... is load-bearing (different PNG bytes); v0.103 fix not effective. lens: %d vs %d",
			len(body), len(body2))
	}
}

// TestPayQR404WhenNoPayloadStored — for orders that were created by
// the pre-v0.103 binary and never got qr_payload populated, /api/pay/qr
...
func TestPayQR404WhenNoPayloadStored(t *testing.T) {
	...
	if strings.HasPrefix(body, "\x89PNG") {
		t.Error("legacy-order: handler rendered a PNG; v0.103 fix bypassed via URL payload")
	}
}
