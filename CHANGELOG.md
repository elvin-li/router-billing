...
## v0.107 — Firewall/walled-garden/deploy correctness: 22.03 apply failure, garden 25h death, List drops a MAC, paid-zone exposure

Deep pass over the MAC whitelist, the captive-portal redirect, the
walled-garden resolver and the OpenWrt deploy scripts. Incorporates
the firewall/portal hardening branch (PR #10) and the nft List
first-element fix independently made on PR #5. Kernel-facing changes
were verified against a live kernel (nft 1.0.9, OpenWrt 23.05
userspace).
...
C. (HIGH) Because B makes every sync a full rebuild, a PARTIAL DNS
failure (one payment domain fails to resolve while others succeed)
would have flushed the failing provider's IPs from the set that same
cycle — one flaky lookup and unpaid clients instantly lose that
payment path, with none of the timeout grace a total outage gets. The
resolver now stands in each domain's last successful answer for up to
25h (matching the element timeout, so a permanently dead domain still
drains on the same schedule). IPv4-mapped answers (`::ffff:a.b.c.d`)
are normalized into the ipv4_addr set instead of discarded.

D. (HIGH) nftables List() dropped the first MAC of every listing (the
...
E. (MED) Sync (nft backend) was flush-then-add as two separate nft
...
F. (MED, security) The fw4 paid zone was created with input=ACCEPT,
...
G. (MED) opkg upgrades opened a free-internet window: prerm runs on
...
H. setup-secure-ssid.sh's "SSID exists, update the key" path had never
...
I. ipk installs brought the Free SSID up OPEN: only install.sh
...
J. Hardening: paid SSIDs get AP client isolation (isolate=1 — the open
paid network is all strangers; the free/friends SSID stays isolate=0),
including the SSID created later by setup-secure-ssid.sh, which
previously skipped it; config.yaml is installed 0600 instead of 0644
(it holds the admin bcrypt hash and WeChat/Alipay merchant keys) by
both install.sh and the ipk build, and install.sh tightens existing
installs.
...
injection; the resolver re-pushes the full list every cycle, keeps a
failing domain's cached IPs (and drains them after 25h), normalizes
IPv4-mapped answers, and leaves the set alone on DNS outage; the ipset
restore script stages+swaps and never touches the live set directly.
