...
## v0.110 — Deploy-script audit: uninstall left a dangling fw4 include, ipk broke image builds
...
Verified: shellcheck clean (remaining SC2015/SC3043 are intentional —
warn-on-failure lists and busybox-ash `local`), dash syntax pass on all
scripts, `make ipk` builds with 0755 scripts in both tarballs, and
postinst/prerm exercised under a fake `IPKG_INSTROOT` (symlinks
created, rc 0, host untouched).
